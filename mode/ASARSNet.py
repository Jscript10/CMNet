...
class Change_detection(nn.Module):
    ...
    def __init__(self, num_classes=1, use_aux=True, fpn_out=48, freeze_bn=False, **_):
        ...
        self.output_fill = nn.Sequential(
            nn.ConvTranspose2d(fpn_out , fpn_out, kernel_size=3, stride = 1,padding=1, bias=False),
            nn.BatchNorm2d(fpn_out),
            nn.ReLU(inplace=True),
...
if __name__ == "__main__":
    net = Change_detection()#.to('cuda')#USE IT WHEN SEE SUMMARY
