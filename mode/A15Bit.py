...
from model2.resnet import *

import model
from Amodel.help_funcs import Transformer, TransformerDecoder, TwoLayerConv2d
...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        if backbone == 'resnet18':
            ...
            self.resnet = model.resnet34(pretrained=True,
                                         replace_stride_with_dilation=[False,True,True])
        elif backbone == 'resnet50':
            self.resnet = model.resnet50(pretrained=True,
                                         replace_stride_with_dilation=[False,True,True])
            expand = 4
        else:
...
class BASE_Transformer(ResNet):
    ...
    def _forward_semantic_tokens(self, x):
        b, c, h, w = x.shape
        spatial_attention = self.conv_a(x)    #  32→4
...
if __name__ == "__main__":
    net = BASE_Transformer(3, 1, 'learned')
